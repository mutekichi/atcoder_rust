...
fn solve<W: Write>(out: &mut W) {
    ...
    let (distances_0, dist_0) = tree.bfs_dist(0);
    let max_dist_0 = distances_0[dist_0];
    let alt_1 = distances_0
        .iter()
        .enumerate()
        .filter(|&(i, d)| *d == max_dist_0 as i64)
        .map(|(i, d)| i)
        .max()
        .unwrap();

    for dist in distances_0 {
        md!(dist);
    }

    let (distances_1, dist_1) = tree.bfs_dist(alt_1);
    let max_dist_1 = distances_1[dist_1];
    let alt_2 = distances_1
        .iter()
        .enumerate()
        .filter(|&(i, &d)| d == max_dist_1 as i64)
        .map(|(i, &d)| i)
        .max()
        .unwrap();
    ...
    let small = min(alt_1, alt_2);
    let large = max(alt_1, alt_2);
    md!(small);
    md!(large);

    tree.build_lca(0);
    for i in 0..n {
        wl!(if tree.dist(i, small) > tree.dist(i, large) {
            small + 1
        } else {
            large + 1
        });
