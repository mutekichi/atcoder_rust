...
fn main() {
    ...
    let mut accum_sums_flat = vec![Mint998::new(0)];
    let mut accum_sums_before = vec![Mint998::new(0)];
    let mut accum_sums_after = vec![Mint998::new(0)];
    for i in 0..n {
        accum_sums_flat.push(accum_sums_flat[i] + A[i]);
    }
    md!(accum_sums_flat.iter().join(" "));
    for i in 0..n {
        accum_sums_before.push(accum_sums_before[i] + A[i] * (i as i64 + 1));
    }
    md!(accum_sums_before.iter().join(" "));
    for i in 0..n {
        accum_sums_after.push(accum_sums_after[i] + A[n - 1 - i] * (i as i64 + 1));
    }
    md!(accum_sums_after.iter().join(" "));
    let mut ans = Mint998::new(0);
    for len in 1..=n {
        md!(len);
        let mut to_sum = Mint998::new(0);
        let stair_len = min(len - 1, n - len);
        md!(stair_len);
        to_sum += accum_sums_before[stair_len];
        md!(accum_sums_before[stair_len]);
        to_sum += accum_sums_after[stair_len];
        md!(accum_sums_after[stair_len]);
        to_sum +=
            (accum_sums_flat[n - stair_len] - accum_sums_flat[stair_len]) * min(len, n + 1 - len);
        md!(accum_sums_flat[n - stair_len], accum_sums_flat[stair_len]);
        to_sum /= len;
        ans += to_sum;
        md!(ans);
    }
