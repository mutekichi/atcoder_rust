...
fn main() {
    ...
    let mut prev_vec = vec![Mint998::from(1)];
    let mut two_pows = vec![Mint998::from(1)];
    for i in 0..n {
        two_pows.push(two_pows[i] * 2);
    }
    for i in 2..=n {
        md!(prev_vec.len(), i);
        let mut new_vec = vec![];
        let coeff = two_pows[i] - 1;
        let mut val = Mint998::new(0);
        let mut val_fac = Mint998::new(1);
        for j in 0..i - 1 {
            val += prev_vec[j] * val_fac;
            val_fac *= 2;
        }
        new_vec.push(val / coeff);
        for j in 0..i - 1 {
            new_vec.push((new_vec[j] + prev_vec[j]) / 2);
        }
        swap(&mut prev_vec, &mut new_vec);
    }
    println!("{}", prev_vec.iter().join(" "));
}
