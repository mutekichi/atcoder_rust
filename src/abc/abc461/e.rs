...
fn main() {
    ...
    let mut st_black = SegmentTree::new(&vec![0; q], |a, b| a + b, 0);
    let mut st_white = SegmentTree::new(&vec![0; q], |a, b| a + b, 0);

    let mut last_black = vec![INF_USIZE; n];
    let mut last_white = vec![0; n];

    let mut ans = 0;
    for i in 0..q {
        let (t, x) = queries[i];
        if t == 1 {
            if last_black[x] == INF_USIZE {
                ans += n;
                st_black.update(i, 1);
            } else {
                ans += st_white.query(last_black[x], i + 1);
                st_black.update(last_black[x], 0);
                st_black.update(i, 1);
            }
            last_black[x] = i;
        } else {
            ans -= st_black.query(last_white[x], i + 1);
            st_white.update(last_white[x], 0);
            st_white.update(i, 1);
            last_white[x] = i;
        }
        println!("{}", ans);
