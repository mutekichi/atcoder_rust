...
fn solve<W: Write>(out: &mut W) {
    ...
    let mut index_list = vec![vec![]; m];
    for i in 0..n {
        let a = A[i];
        index_list[a].push(i);
        if i > 0 {
            inversion_number += st.query(a + 1, m);
        }
        st.update(a, st.get(a) + 1);
    }
    for i in 0..m {
        wl!(inversion_number);
        let target = m - i - 1;
        md!(i, m, target);
        let len = index_list[target].len();
        for i in 0..len {
            let j = index_list[target][i];
            inversion_number += j;
            inversion_number += len - i - 1;
            inversion_number -= n - j - 1;
            inversion_number -= i;
        }
    }
}
