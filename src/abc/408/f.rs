...
fn get_next_range(
    ...
    (i.saturating_sub(r), min(n, i + r + 1))
}
...
fn solve<W: Write>(out: &mut W) {
    ...
    let min_steps = vec![-1; n];
    let mut st = SegmentTree::new(
        &min_steps,
        |a, b| max(a, b),
        -1,
    );
    let mut inv_H = vec![0; n];
    for i in 0..n {
        inv_H[H[i]] = i;
    }
    ...
    for i in 0..d {
        ...
        let next_idx = inv_H[next_height];
        let (next_l, next_r) = get_next_range(n, next_idx, r);
        let next_max = st.query(next_l, next_r);
        md!(next_l, next_r);
        let next_value = if next_max == -1 { 0 } else { next_max + 1 };
        md!(i, inv_H[i], next_value);
        queue.push_back((next_height, next_value));
    }
    let mut ans = 0;

    while let Some((height, value)) = queue.pop_front() {
        st.update(inv_H[height], value);
        md!(height, inv_H[height], value);
        ans = max(value, ans);
        let next_height = height + d;
        if next_height >= n {
            continue;
        }
        let next_idx = inv_H[next_height];
        let (next_l, next_r) = get_next_range(n, next_idx, r);
        let next_max = st.query(next_l, next_r);
        let next_value = if next_max == -1 { 0 } else {
            next_max + 1
        };
        md!(height, inv_H[height], next_value);
        queue.push_back((next_height, next_value));
    }
    wl!(ans);
