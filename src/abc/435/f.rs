...
fn solve<W: Write>(out: &mut W) {
    ...
    let data: Vec<(usize, usize)> = vec_a.iter().enumerate().map(
        |(i, v)| { (*v, i) }
    ).collect::<Vec<(usize, usize)>>();

    let identity = (0 as usize, 0 as usize);
    ...
    let start = st.query(0, n);

    let mut q = VecDeque::new();
    q.push_back((start.1, 0, n));

    while !q.is_empty() {
        if let Some(v) = q.pop_front() {
            let (idx, left, right) = v;
            
            st.update(idx, identity);

            if idx - left > 0 {
                let left_max = st.query(left, idx);
                graph[idx].push(left_max.1);
                graph[left_max.1].push(idx);
                q.push_back((left_max.1, left, idx));
            }
            if right - idx > 1 {
                let right_max = st.query(idx + 1, right);
                graph[idx].push(right_max.1);
                graph[right_max.1].push(idx);
                q.push_back((right_max.1, idx + 1, right));
            }
        }
    }

    wl!(dp(start.1, INF_USIZE, &graph));
...
fn dp(
    ...
    for next in &graph[v] {
        ...
        let next_value = dp(*next, v, graph) + if v > *next { v - *next } else { *next - v };
