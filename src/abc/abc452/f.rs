...
fn main() {
    input! {
        n: usize, k: usize, P: [Usize1; n],
    }

    let mut st = SegmentTree::new(&vec![0; n], |a, b| a + b, 0);
    ...
    let mut inversion_set = BTreeSet::new();
    ...
    loop {
        ...
        while inversions <= k {
            inversions += st.query(P[right], n);
            if inversions == k {
                inversion_set.insert((left, right));
            }
            st.update(P[right], 1);
            right += 1;
            if right == n {
    ...
    for (left, right) in inversion_set.iter() {
        println!("{} {}", *left, *right);
    }
    println!("{}", inversion_set.len());
}
