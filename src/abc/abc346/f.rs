...
fn main() {
    ...
    let mut st = vec![SegmentTree::new(&vec![0usize; s.len()], |a, b| a + b, 0usize,); 26];
    for i in 0..s.len() {
        let c = s[i];
        let val = st[c].get(i) + 1;
        st[c].update(i, val);
    }
    let mut ok = -1;
    let mut ng = INF_I64;
...
fn check(
    ...
    s: &Vec<usize>,
    t: &Vec<usize>,
    st: &Vec<SegmentTree<usize, impl Fn(usize, usize) -> usize>>,
    k: i64,
) -> bool {
    md!(k);
    if k == 0 {
    ...
    for &c in t {
        md!(c, idx, cnt);
        let mut rem = k;
        let latter = st[c].query(idx, s.len()) as i64;
        md!(latter, rem);
        if latter >= rem {
            idx = st[c].max_right(idx, |e| e < rem as usize) + 1;
        } else {
            let char_count = st[c].query(0, s.len()) as i64;
            if char_count == 0 {
                return false;
            }
            rem -= latter;
            ...
            if rem > 0 {
                ...
                rem = rem % char_count;
                rem += 1;
                idx = st[c].max_right(0, |e| e < rem as usize) + 1;
            }
        }
        if cnt >= n {
            return false;
