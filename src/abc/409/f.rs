...
fn manhattan(
    ...
    (XY[u].0 - XY[v].0).abs() + (XY[u].1 - XY[v].1).abs()
}
...
fn solve<W: Write>(out: &mut W) {
    ...
    for i in 0..(n-1) {
        for j in (i+1)..n {
            pq.push(Reverse((manhattan(&XY, i, j), i, j)));
        }
    }
    ...
    for _ in 0..q {
        ...
        if qtype == 1 {
            ...
            XY.push((a, b));
            for i in 0..(n-1) {
                pq.push(Reverse((manhattan(&XY, i, n - 1), i, n - 1)));
            }
        } else if qtype == 2 {
            md!(qtype);
            let mut min_dist = INF_I64;
