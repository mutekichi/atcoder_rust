...
fn solve<W: Write>(out: &mut W) {
    ...
    for _ in 0..t {
        ...
        let mut st = SegmentTree::new(
            &vec![Some((-1i64, -1i64)); 2 * n],
            |a, b| match(a, b) {
                (Some(a), Some(b)) => Some((a.0 + b.0, max(b.1, a.1 + b.0))),
                (a, b) => a.or(b),
            },
            None,
        );

        let mut value_idx = BinaryHeap::new();
        for i in 0..(2 * n) {
            value_idx.push((A[i], i));
        }
        while let Some((value, idx)) = value_idx.pop() {
            if idx == 2 * n - 1 {
                continue;
            }
            st.update(idx, Some((1, 1)));
            let max_suffix_sum = st.query(0, 2 * n).unwrap().1;
            if max_suffix_sum > 0 {
                md!("rejected", value, idx, max_suffix_sum);
                st.update(idx, Some((-1, -1)));
            }
            else {
                md!("accepted", value, idx, max_suffix_sum);
                ans += value;
            }
        }
        for i in 0..(2 * n) {
            mep!(if st.query(i, i + 1).unwrap().0 == 1 {'('} else {')'});
        }
        wl!(ans);
    }
}
