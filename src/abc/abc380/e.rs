...
fn main() {
    ...
    let data = vec![false; n + 2];
    let mut st = SegmentTree::new(&data, |a, b| a & b, true);
    let mut colors = (0..n + 2).collect::<Vec<_>>();
    let mut counts = vec![1; n + 2];
    for _ in 0..q {
        input! {
            qtype: usize,
        }
        if qtype == 1 {
            ...
            let left = st.min_left(x, |a| a);
            let right = st.max_right(x, |a| a);
            md!(left, right);
            counts[c] += right - left + 1;
            counts[colors[left]] -= right - left + 1;
            if colors[left - 1] == c {
                st.update(left - 1, true);
            }
            colors[left] = c;
            if colors[right + 1] == c {
