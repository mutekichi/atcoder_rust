...
fn solve<W: Write>(out: &mut W) {
    ...
    for k in 1..320000i128 {
        let mut ok = 1000000000i128;
        let mut ng = -1i128;
    ...
    eprintln!("here");
    let M = 1500000i128;
    let mut i3s = vec![0; M as usize];
    let mut map = BTreeMap::new();
    for i in 1..M {
        i3s[i as usize] = i * i * i;
        map.insert(i3s[i as usize], i);
    }
    for i in 1..M {
        let to_find = n + i3s[i as usize];
        if let Some(value) = map.get(&to_find) {
            println!("{} {}", value, i);
            return;
        }
    }
