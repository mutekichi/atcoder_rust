...
fn main() {
    ...
    let comb: Combination<C998244353> = Combination::new(1000010);
    dfs(
        0, INF_USIZE, &tree, &mut ans,
        // &C.iter().map(|v| Mint998::new(*v)).collect::<Vec<_>>(),
        // &D.iter().map(|v| Mint998::new(*v)).collect::<Vec<_>>(),
        &C, &D, &comb,
    );
    println!("{}", ans);
}
...
fn dfs(
    ...
    for &nv in &tree[v] {
        if nv != from {
            choices += dfs(nv, v, tree, ans, C, D, comb);
            md!(v, nv, choices)
        }
    }
    md!(v, choices, D[v]);
    if choices < D[v] {
        *ans = Mint998::new(0);
        ...
        // *ans = *ans * comb.ncr(choices as usize, D[v] as usize);
        for i in 0..D[v] {
            *ans = *ans * (choices - i); 
        }
        for i in 0..D[v] {
            *ans = *ans / Mint998::new(i + 1);
        }
    }
    return choices - D[v];
}
