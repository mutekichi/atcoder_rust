...
fn main() {
    ...
    let mut min_segtree = SegmentTree::new(&vec![0; n + q + 2], |a, b| min(a, b), INF_USIZE);
    let mut cnt_segtree = SegmentTree::new(&vec![0; n + q + 2], |a, b| a + b, 0);
    cnt_segtree.update(0, n);
    let mut base = 0;
    for _ in 0..q {
        input! {
            qtype: usize,
        ...
        if qtype == 1 {
            let next_cnt = min_segtree.get(x) + 1;
            min_segtree.update(x, next_cnt);
            cnt_segtree.update(
                next_cnt.saturating_sub(1),
                cnt_segtree
                    .get(next_cnt.saturating_sub(1))
                    .saturating_sub(1),
            );
            cnt_segtree.update(next_cnt, cnt_segtree.get(next_cnt) + 1);
            if min_segtree.query(0, n) > base {
                base += 1;
            }
        } else {
            let amount = x + 1 + base;
            ...
            println!(
                "{}",
                n.saturating_sub(cnt_segtree.query(0, min(n + q + 2, amount)))
            );
        }
    }
}
