...
fn solve<W: Write>(out: &mut W) {
    ...
    let trans_table = {
        let mut table = vec![vec![INF_USIZE; 26]; 1 << n];
        for i in 0..26 {
            for j in 0..1 << n {
    ...
    let mut dp_table = vec![vec![Mint998::new(0); 1 << n]; m + 1];
    dp_table[0][0] = Mint998::new(1);
    for i in 0..m {
        for j in 0..26 {
            for k in 0..1 << n {
                let to = trans_table[k][j];
                let to_add = dp_table[i][k];
                dp_table[i + 1][to] += to_add;
            }
        }
    }
    let mut ans = vec![Mint998::new(0); n + 1];
    for i in 0usize..1 << n {
        let popcount = i.count_ones() as usize;
        ans[popcount] += dp_table[m][i];
    }
    for a in ans {
        println!("{}", a);
    }
