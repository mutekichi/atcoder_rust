...
fn solve<W: Write>(out: &mut W) {
    ...
    for (x, y) in XY {
        ...
        let &(s_value, s_index) = set.range((x, 0)..).next().unwrap();
        md!(s_value, s_index);
        let mut ok = s_index - 1;
        md!(A[ok]);
        let mut ng = A.len();
        ...
        while ok.abs_diff(ng) > 1 {
            ...
            let mid_value = A[mid];
            md!(mid, mid_value);
            md!(x + y + (mid - s_index) as i64, mid_value);
            if x + y + (mid - s_index) as i64 >= mid_value + 1 {
                ok = mid;
            } else {
                ng = mid;
            }
