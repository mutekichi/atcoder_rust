...
fn main() {
    input! {
        n: usize, m: usize, y: i64,
        UVT: [(Usize1, Usize1, i64); m],
        X: [i64; n],
    }
    let mut graph = WeightedGraph::new(n + 1);
    for (u, v, t) in UVT {
        graph.add_undirected_edge(u, v, t * 2);
    }
    for i in 0..n {
        graph.add_undirected_edge(i, n, X[i] * 2 + y);
    }
    let dists = graph.dijkstra(0);
    println!("{}", dists.iter().skip(1).take(n - 1).map(|d| d / 2).join(" "));
}
