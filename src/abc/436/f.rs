...
fn solve<W: Write>(out: &mut W) {
    ...
    let mut ans: i64 = 0;
    let mut inv_b = vec![0; n];
    for (i, b) in B.iter().enumerate() {
        md!(i, *b);
        inv_b[*b] = i;
    }

    let data_count = vec![1; n];
    let mut stcount = SegmentTree::new(
        &data_count,
        |a, b| { a + b },
        0,
    );
    ...
    for i in 0..n {
        let val = n - i - 1;
        let idx = inv_b[val];
        let lefts = stcount.query(0, idx + 1) as i64;
        let rights = stcount.query(idx, n) as i64;
        ans += lefts * rights;
        md!(idx, val, lefts, rights);
