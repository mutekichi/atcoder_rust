...
fn solve<W: Write>(out: &mut W) {
    ...
    let data_max: Vec<_> = A.into_iter().enumerate().map(
        |(i, x)| { (x, i) }
    ).collect();
    let st_max = SegmentTree::new(
        &data_max,
        |a, b| { max(a, b) },
        (0, 0)
    );
    let mut data_to_sub = vec![0i64; n + 1];
    ...
    while let Some(front) = q.pop_front() {
        ...
        let (value, index) = st_max.query(left, right);
        md!(value, index);
        q.push_back((left, index));
        if index == right - 1 {
            continue;
        }
        let (next_value, next_index) = st_max.query(index + 1, right);
        md!(next_value, next_index);
        md!(right - index, value - next_value);
        md!(next_index - index);
        data_to_sub[right - index] += value - next_value;
        data_to_sub[next_index - index] -= value - next_value;
        q.push_back((index, next_index));
        q.push_back((next_index, right));
    }
    for i in (0..n).rev() {
        data_to_sub[i] += data_to_sub[i + 1]
    }
    for i in 2..=n {
        md!(i, data_to_sub[i]);
    }
    let mut ans = vec![-1; n + 1];
    ans[n] = st_max.query(0, n).0;
    for i in (1..n).rev() {
        ans[i] = ans[i + 1] + st_max.query(0, i).0 - data_to_sub[i + 1];
    }
    for i in 1..=n {
        wl!(ans[i]);
    }
