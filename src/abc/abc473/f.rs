...
fn main() {
    ...
    let mut diff = 0i64;
    let mut data: Vec<S> = vec![];
    data.push(S { val: diff });
    for i in 0..n {
        if T[i] == 'A' {
            diff += 1;
        } else {
    ...
    for _ in 0..q {
        ...
        if q == 1 {
            ...
            let prev_diff = st.get(l - 1);
            md!(st.prod(l, r + 1).val);
            md!(prev_diff.val);
            if st.prod(l, r + 1).val >= prev_diff.val {
                println!("Yes");
            } else {
                println!("No");
            }
    ...
    C: Fn(U, U) -> U,
{
    ...
    fn update(
        ...
        self.tree[k] =
            (self.op_tree)(self.tree[2 * k], self.tree[2 * k + 1]);
    }
