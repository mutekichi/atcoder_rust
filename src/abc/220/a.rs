...
fn solve<W: Write>(out: &mut W) {
    ...
    vec_xi.sort_unstable();
    vec_yi.sort_unstable();

    let mut graph = WeightedGraph::new(n);

    for i in 0..(n-1) {
        graph.add_edge(vec_xi[i].1, vec_xi[i + 1].1, vec_xi[i + 1].0 - vec_xi[i].0);
        graph.add_edge(vec_yi[i].1, vec_yi[i + 1].1, vec_yi[i + 1].0 - vec_yi[i].0);
    }
