...
fn solve<W: Write>(out: &mut W) {
    ...
    if m == 1 {
        let count = A.iter().filter(|&&e| e == 1).count();
    ...
    let pf = prime_factorize(m);
    let pf_total = pf.iter().map(|&(p, n)| p.pow(n as u32)).collect::<Vec<_>>();
    let l = pf.len();
    assert!(l <= 13);
    let A = A
        .iter()
        ...
        .collect::<Vec<_>>();
    let n = A.len();
    let mut dp = vec![Mint998::new(0); 1<<l];
    dp[0] += 1;
    for i in 0..n {
        for j in (0..(1 << l)).rev() {
            let val = dp[j];
...
fn prime_factorize(n: i64) -> Vec<(i64, usize)> {
    let mut res = vec![];
    let mut n = n;
    for i in 2.. {
        if i * i > n {
