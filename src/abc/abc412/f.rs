...
fn solve<W: Write>(out: &mut W) {
    ...
    A[c] += 1;
    let total = A.iter().sum::<i64>() - 1;
    md!(total);
    let mut map: BTreeMap<i64, i64> = BTreeMap::new();
    for &a in &A {
        *map.entry(a).or_insert(0) += 1;
    }
    let mut key_value_list = vec![];
    let mut f_list = vec![];
    for &key in map.keys().rev() {
        key_value_list.push((key, map[&key]));
    }
    let len = key_value_list.len();
    for i in 0..len {
        let mut ans = Mint998::new(0);
        let (key, value) = key_value_list[i];
        md!(key, value);
        if key != 1 {
            ans += Mint998::new(key - 1) / Mint998::new(total);
            md!(key - 1, total);
        }
        let mut larger_count = 0;
        for j in 0..i {
            let (l_key, l_value) = key_value_list[j];
            ans += (f_list[j] + 1) * Mint998::new(l_key * l_value) / Mint998::new(total);
            larger_count += l_key * l_value;
            md!(f_list[j] + 1, l_key * l_value, total);
        }
        let remain = total - larger_count - key + 1;
        ans += Mint998::new(remain) / Mint998::new(total);
        if remain != 0 {
            ans *= Mint998::new(total);
            ans /= Mint998::new(total - remain);
            md!(total, total - remain);
        }
        md!(key, A[c]);
        if key == A[c] {
            wl!(ans);
