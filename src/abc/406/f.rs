...
fn dfs(
    v: usize,
    graph: &Vec<Vec<usize>>,
    current_virtual_index: &mut usize,
    node_idx_to_virtual_index: &mut Vec<usize>,
    deepest_virtual_index: &mut Vec<usize>,
) {
    md!(*current_virtual_index);
    for &nv in &graph[v] {
        md!(nv);
        if node_idx_to_virtual_index[nv] != INF_USIZE {
            md!(node_idx_to_virtual_index[nv]);
            continue;
        } else {
            *current_virtual_index += 1;
            node_idx_to_virtual_index[nv] = *current_virtual_index;
            dfs(
                nv,
                graph,
                current_virtual_index,
                node_idx_to_virtual_index,
                deepest_virtual_index,
            );
        }
    }
    deepest_virtual_index[node_idx_to_virtual_index[v]] = *current_virtual_index;
}
...
fn solve<W: Write>(out: &mut W) {
    ...
    let mut node_idx_to_virtual_index = vec![INF_USIZE; n];
    let mut current_virtual_index = 0;
    let mut deepest_virtual_index = vec![INF_USIZE; n];
    node_idx_to_virtual_index[0] = current_virtual_index;

    dfs(
        0,
        &graph,
        &mut current_virtual_index,
        &mut node_idx_to_virtual_index,
        &mut deepest_virtual_index
    );
    ...
    for _ in 0..q {
        ...
        if qtype == 1 {
            ...
            let virtual_index = node_idx_to_virtual_index[x];
            st.update(virtual_index, st.get(virtual_index) + w);
            weight_total += w;
        }
        else {
            input! {
            ...
            let (u, v) = UV[y];
            let vindex_u = node_idx_to_virtual_index[u];
            let vindex_v = node_idx_to_virtual_index[v];
            let small = min(vindex_u, vindex_v);
            let large = max(vindex_u, vindex_v);
            let weight_sum = st.query(large, deepest_virtual_index[large] + 1);
            wl!((weight_total - 2 * weight_sum).abs());
        }
    }
}
