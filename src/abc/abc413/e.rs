...
fn solve<W: Write>(out: &mut W) {
    ...
    for _ in 0..t {
        input! {
            n: u32,
            data: [i64; 2usize.pow(n) as usize],
        }
        let array = data.into_iter().map(|x| vec![x]).collect::<Vec<_>>();
        let st = SegmentTree::new(
            &array,
            |x, y| { 
                if x.is_empty() { return y.clone();}
                else if y.is_empty() { x.clone() }
                else {
                    if x[0] < y[0] { [x.clone(), y.clone()].concat()}
                    else {[y.clone(), x.clone()].concat()}
                }
            },
            vec![],
        );
        let ans = st.query(0, 2usize.pow(n) as usize).iter().map(
            |x| x.to_string()
        ).join(" ");
        wl!(ans);
    }
