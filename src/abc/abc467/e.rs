...
fn main() {
    ...
    let A = A.windows(2).map(|w| (w[0] + w[1]) % m).collect::<Vec<_>>();
    md!(A.iter().join(" "));
    let diffs = A
        .into_iter()
        .zip(B)
    ...
    for diff in diffs {
        ...
        data.push((diff + m - prev) % m);
    }
    md!(data.iter().join(" "));

    let mut values = BTreeSet::new();
    let mut counter1 = BTreeMap::new();
    let mut counter2 = BTreeMap::new();
    let mut sum = 0;

    for i in 0..n {
        sum += data[i];
        if i % 2 == 0 {
            *counter1.entry(data[i]).or_insert(0) += 1;
            values.insert(data[i]);
        } else {
            *counter2.entry((m - data[i] - 1) % m).or_insert(0) += 1;
            values.insert((m - data[i] - 1) % m);
        }
    }
    let mut ans = INF_I128;
    let mut prev_val = m;
    let mut total = 0;
    let diff = if n % 2 == 1 { 1 } else { 0 };
    for &value in values.iter().rev() {
        assert!(value < m);
        let count1 = counter1.get(&value).unwrap_or(&0).clone();
        let count2 = counter2.get(&value).unwrap_or(&0).clone();
        let val_diff = prev_val - value;
        total += diff * val_diff;
        total += count2 * m;
        total -= count1 * m;
        prev_val = value;
