...
fn solve<W: Write>(out: &mut W) {
    ...
    let mut prod = Mint998::new(0);

    let mut lefts: BTreeSet<usize> = (0..n).collect();

    for me in mes {
        let (val, index) = me;
        lefts.remove(&index);
        lower_counts[index] += 1;
        prod *= Mint998::new(lower_counts[index]);
        prod /= Mint998::new(lower_counts[index] - 1);
        if prod.val() == 0 && lefts.is_empty() {
            prod += 1;
            for c in &lower_counts {
                prod *= Mint998::new(*c);
            }
        }
        ans += Mint998::new(val) * prod / Mint998::new(lower_counts[index]);
        md!(ans, val, index, val, lower_counts[index]);
    }
    let inv6 = Mint998::new(6).inv();
    for i in 0..n {
        ans *= inv6;
    }
    wl!(ans);
