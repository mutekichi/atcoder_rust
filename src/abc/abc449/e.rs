...
fn main() {
    ...
    let mut overs = vec![];
    let mut total_over_count = 0i64;
    for i in 0..m {
        if counter[i] != max_count {
            overs.push((counter[i] as i64, i));
            total_over_count += (max_count - counter[i]) as i64;
        }
    }
    ...
    overs.sort_unstable();
    overs.push((max_count as i64, INF_USIZE));
    md!(overs.len());

    let mut total_rem = 0;
    let mut over_idx = 0;
    let mut over_count = 0;
    let mut current_rem_to_add = 0;
    let mut st = SegmentTree::new(&vec![0; m], |a, b| a + b, 0);

    for (x, i) in queries {
        if x < n as i64 {
            md!(x, i);
            ans[i] = A[x as usize] + 1;
        } else if x < n as i64 + total_over_count {
            let rem = x - n as i64;
            while rem >= total_rem + current_rem_to_add {
                total_rem += current_rem_to_add;
                loop {
                    let (count, val) = overs[over_idx];
                    over_count += 1;
                    st.update(val, 1);
                    if overs[over_idx].0 != overs[over_idx + 1].0 {
                        break;
                    }
                    over_idx += 1;
                }
                current_rem_to_add = (overs[over_idx + 1].0 - overs[over_idx].0) * over_count;
                over_idx += 1;
            }
            let rem_2 = rem - total_rem;
            let amari = rem_2 % over_count;
            md!(x, rem_2, amari, over_count);
            for i in 0..m {
                md!(i, st.get(i));
            }
            ans[i] = st.max_right(0, |e| e < amari + 1) + 1;
            md!(ans[i]);
        } else {
            let rem = x - (n as i64 + total_over_count);
            ans[i] = (rem % m as i64 + 1) as usize;
        }
    }
