...
fn make_cartesian_tree(
    st: &SegmentTree<(usize, usize), impl Fn((usize, usize), (usize, usize)) -> (usize, usize)>,
    n: usize,
    ...
    if top != l {
        let left_top = st.query(l, top).1;
        graph[top].push(left_top);
        make_cartesian_tree(st, n, graph, l, top, left_top);
    }
    if top != r - 1 {
        let right_top = st.query(top + 1, r).1;
        graph[top].push(right_top);
        make_cartesian_tree(st, n, graph, top + 1, r, right_top);
    }
}
...
fn solve<W: Write>(out: &mut W) {
    ...
    let data = P
        ...
        .map(|(i, v)| (*v, i))
        .collect::<Vec<_>>();
    let st = SegmentTree::new(&data, |l, r| max(l, r), (0, INF_USIZE));
    let root = st.query(0, n).1;

    let mut graph = vec![vec![]; n];
    make_cartesian_tree(&st, n, &mut graph, 0, n, root);
