...
fn main() {
    ...
    for _ in 0..t {
        ...
        let mut zero_edges = vec![];
        let mut one_edges = vec![];
        for i in 0..m {
            let (a, b, c) = ABC[i];
            if c == 0 {
                zero_edges.push((a, b, i));
            } else {
                one_edges.push((a, b, i));
            }
        }
        let mut edges = vec![false; m];
        let mut uf = UnionFind::new(n);
        let mut edge_set = vec![BTreeSet::new(); n];

        for &(a, b, i) in &zero_edges {
            if edges.len() == n - 1 {
                break;
            }
            if !uf.same(a, b) {
        ...
        let mut one_edge_idx = 0;
        let mut one_count = 0;
        for &(a, b, i) in &one_edges {
            if edges.len() == n - 1 {
                continue;
            }
            if !uf.same(a, b) {
        ...
        for j in one_edge_idx..one_edges.len() {
            if one_count == k {
                break;
            }
            let (a, b, i) = one_edges[j];
            if !edge_set[a].is_empty() {
                let j = edge_set[a].pop_last().unwrap();
                edges[j] = false;
                edges[i] = true;
                let (x, y, c) = ABC[j];
                if a == x {
                ...
                one_count += 1;
            } else if !edge_set[b].is_empty() {
                let j = edge_set[b].pop_last().unwrap();
                edges[j] = false;
                edges[i] = true;
                let (x, y, c) = ABC[j];
                if a == x {
