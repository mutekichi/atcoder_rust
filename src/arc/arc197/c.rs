    ...
    F: Fn(T, T) -> T,
{
    ...
    /// # Complexity
    /// - O(log N)
    pub fn update(&mut self, p: usize, value: T) {
...
fn solve<W: Write>(out: &mut W) {
    ...
    let LIMIT = (1<<22) as usize;
    md!(LIMIT);
    let data = vec![1usize; LIMIT];
    let mut st = SegmentTree::new(
        &data,
        |a, b| a + b,
        0,
    ...
    for &(a, b) in &AB {
        ...
        let mut ok: usize = LIMIT;
        let mut ng = 0;
        while ok.abs_diff(ng) > 1 {
            let mid = (ok + ng) / 2;
            let count = st.query(0, mid + 1);
            if count >= b {
                ok = mid;
            } else {
                ng = mid;
